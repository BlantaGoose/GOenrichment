...
Kyoumi1 <- read_tsv("../cafe/summary/0708summary/SpecificGF.txt", col_names = "Orthogroup") %>%
...
##加工によってすべてのOGsを取り出す。
##fams.csvファイルに、terminalのviコマンドで一行目を削除したりしておこう。名前もorthogroupsと共通にしよう。
moto <- read_csv("../cafe/summary/0708summary/0708summary_fams.csv", col_names = FALSE) %>%
  separate(X1, into = c("Node","X1"), sep = ":")

outgroupmoto <- moto %>% filter(Node %in% c("Taeniopygia_guttata_lsi", "Ficedula_albicollis_lsi", "Parus_major_lsi")) %>% 
  t()
outgroupmoto2 <- outgroupmoto %>% str_remove("\\[.+\\]")
...
#Falb <- outgroup_3[outgroup_3$species == "Ficedula_albicollis_lsi",]
#Pmaj <- outgroup_3[outgroup_3$species == "Parus_major_lsi",]
#Tgut <- outgroup_3[outgroup_3$species == "Taeniopygia_guttata_lsi",]
Falb <- outgroupmoto3[outgroupmoto3]
...
attributes[grep("refseq", attributes[,1]),] ##ensembl,go_id,seqで検索


##3.それじゃあ変換しよう
##念の為三種のアノテーション情報を用いる
resFa <- getBM(attributes =c("refseq_peptide_predicted", "entrezgene_id"),  #attributesに入れる値は、listAttributes()で調べたやつ
               filters = "refseq_peptide_predicted", #filters に入れる値は、listFilters()で探したやつ
               values = Falb$refseq, #valuesには、変換元のデータが入っている列を指定。
               mart = fa)

resPm <- getBM(attributes =c("refseq_peptide_predicted", "entrezgene_id"),  #attributesに入れる値は、listAttributes()で調べたやつ
               filters = "refseq_peptide_predicted", #filters に入れる値は、listFilters()で探したやつ
               values = Pmaj$refseq, #valuesには、変換元のデータが入っている列を指定。
               mart = pm)

resTg <- getBM(attributes =c("refseq_peptide_predicted", "entrezgene_id"),  #attributesに入れる値は、listAttributes()で調べたやつ
               filters = "refseq_peptide_predicted", #filters に入れる値は、listFilters()で探したやつ
...
Kyoumiannotation <- full_join(outgroup_3, res, 
  ...
  na.omit() %>%
  dplyr::select(Orthogroup, entrezgene_id)
...
#Pti <- read_table("../cafe/output/0708result/0708cafe.cafe",
...
tg_mohou <- new("topGOdata",
                ...
                mapping = "org.Tg.eg.db",
                ID = "ENTREZ")
class(Genes[1])
class(named_scores[1])
...
entrez_ids = mappedkeys(org.Gg.egGO)
